...
type EnhancedObjectRunner struct {
	...
	commonDataRepo       *model.CommonDataRepository
...
func NewEnhancedObjectRunner() *EnhancedObjectRunner {
	runner := &EnhancedObjectRunner{
		...
		commonDataRepo:       model.NewCommonDataRepository(),
...
func (r *EnhancedObjectRunner) ValidateFile(fileName, codespace, reportID string, xmlData []byte, xmlDoc *xmlquery.Node) ([]types.ValidationIssue, error) {
	...
	ctx, err := model.NewObjectValidationContext(fileName, codespace, reportID, xmlData, xmlDoc)
	if err != nil {
...
func (r *EnhancedObjectRunner) ValidateDataset(files []FileData) ([]types.ValidationIssue, error) {
	...
	if r.enableDataCollection {
		...
		r.commonDataRepo = model.NewCommonDataRepository()
	}

	// First pass: collect data from all files
	var contexts []*model.ObjectValidationContext
	...
	for _, file := range files {
		...
		// Create object validation context
		ctx, err := model.NewObjectValidationContext(file.FileName, file.Codespace, file.ReportID, file.Content, xmlDoc)
		if err != nil {
...
func (r *EnhancedObjectRunner) validateCrossFileConsistency(contexts []*model.ObjectValidationContext) []types.ValidationIssue {
...
func (r *EnhancedObjectRunner) validateNetworkConnectivity(contexts []*model.ObjectValidationContext, ntc *NetworkTopologyCollector) []types.ValidationIssue {
	var issues []types.ValidationIssue
...
func (r *EnhancedObjectRunner) validateServiceConsistency(contexts []*model.ObjectValidationContext) []types.ValidationIssue {
	var issues []types.ValidationIssue
