...
//
// This package validates NetEX (Network Exchange) files against the Nordic NeTEx Profile,
...
import (
	...
	"github.com/theoremus-urban-solutions/netex-validator/cache"
	"github.com/theoremus-urban-solutions/netex-validator/config"
	"github.com/theoremus-urban-solutions/netex-validator/context"
	"github.com/theoremus-urban-solutions/netex-validator/idvalidation"
	...
	xsdpkg "github.com/theoremus-urban-solutions/netex-validator/schema"
	"github.com/theoremus-urban-solutions/netex-validator/types"
	"github.com/theoremus-urban-solutions/netex-validator/validator"
	"github.com/theoremus-urban-solutions/netex-validator/xpath"
)
...
// NetEX files and content against the Nordic NeTEx Profile.
//
...
type NetexValidator struct {
	...
	runner          *validator.EnhancedNetexValidatorsRunner
	codespace       string
...
func NewWithOptions(opts *ValidationOptions) (*NetexValidator, error) {
	...
	var validationCache cache.ValidationCache
	if opts.EnableValidationCache {
		...
		cacheOpts := &cache.MemoryCacheOptions{
			MaxEntries: maxEntries,
			MaxBytes:   maxMemoryBytes,
		}
		memoryCache := cache.NewMemoryValidationCache(cacheOpts)
...
func (v *NetexValidator) ValidateContent(content []byte, filename string) (*ValidationResult, error) {
	...
	// Run validation using the correct signature
	report, err := v.runner.ValidateContent(filename, v.codespace, content, false, false)
	if err != nil {
		return &ValidationResult{
...
func (v *NetexValidator) validateContentWithCaching(content []byte, filename string, startTime time.Time) (*ValidationResult, error) {
	...
	if v.validationCache != nil {
		fileHash = cache.CalculateFileHash(content)
...
func (v *NetexValidator) initializeRunner(opts *ValidationOptions) error {
	// Create builder
	builder := validator.NewEnhancedNetexValidatorsRunnerBuilder()
	...
	if !opts.SkipValidators {
		...
		if opts.RuleOverrides != nil && len(opts.RuleOverrides) > 0 {
		...
		if opts.SeverityOverrides != nil && len(opts.SeverityOverrides) > 0 {
			for i := range enabled {
		...
		xrules := make([]xpath.XPathValidationRule, 0, len(enabled))
		for _, r := range enabled {
			xrules = append(xrules, NewSimpleXPathRule(r))
		}
		if len(xrules) > 0 {
			xpathValidator := xpath.NewXPathRuleValidator(xrules)
			builder = builder.WithXPathValidators([]interfaces.XPathValidator{xpathValidator})
		}
	}

	// Add ID validator
	idRepo := idvalidation.NewNetexIdRepository()
	idExtractor := idvalidation.NewNetexIdExtractor()
	idValidator := idvalidation.NewNetexIdValidator(idRepo, idExtractor)
